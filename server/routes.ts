...
export async function registerRoutes(app: Express) {
  ...
  // Dashboard summary endpoint to get various metrics for the organization
  app.get("/api/organizations/:orgId/dashboard-summary", async (req, res) => {
    try {
      if (!req.user) {
        return res.status(401).json({ message: "Authentication required" });
      }
      ...
      // Only allow users who belong to this organization to access its data
      if (req.user.organizationId !== orgId && req.user.role !== "admin") {
        return res.status(403).json({ message: "Not authorized to access this organization's data" });
      }
      
      // Get all camps for this organization
      const allCamps = await storage.listCamps(orgId);
      
      // Get today's date for filtering
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      
      // Prepare our summary data
      const summary = {
        // Camp stats
        campCounts: {
        ...
        sessionsByDate: {} as Record<string, { campIds: number[], sessionCount: number }>,
      };
      
      // Get the past 48 hours for recent registrations
      const recentCutoff = new Date();
      recentCutoff.setHours(recentCutoff.getHours() - 48);
      
      // Process all camps
      const campPromises = allCamps.map(async (camp) => {
        ...
        // Categorize the camp
        if (today < campStartDate) {
          summary.campCounts.upcoming++;
        } else if (today > campEndDate) {
          summary.campCounts.completed++;
        } else {
          summary.campCounts.active++;
        }
        
        // Check if registration is open
        if (today >= regStartDate && today <= regEndDate) {
          summary.campCounts.registrationOpen++;
        }
        
        // Get registrations for this camp
        const registrations = await storage.getRegistrationsByCamp(camp.id);
        ...
        const recentRegs = registrations.filter(reg => {
          ...
          return regDate >= recentCutoff;
        });
        summary.recentRegistrations += recentRegs.length;
        
        // Get sessions for this camp
        const campSessions = await storage.getCampSessions(camp.id);
        
        // Find today's sessions
        const todaySessions = campSessions.filter(session => {
          const sessionDate = new Date(session.sessionDate);
          return sessionDate.toDateString() === today.toDateString();
        });
        
        // Add today's sessions with camp info
        todaySessions.forEach(session => {
          summary.todaySessions.push({
            ...session,
            campName: camp.name,
            campId: camp.id
          });
        ...
        // Process session calendar data (for the next 60 days)
        const endCalendarDate = new Date();
        endCalendarDate.setDate(today.getDate() + 60);
        
        campSessions.forEach(session => {
          const sessionDate = new Date(session.sessionDate);
          
          // Only include dates within our range
          if (sessionDate >= today && sessionDate <= endCalendarDate) {
            const dateKey = sessionDate.toISOString().split('T')[0];
            
            if (!summary.sessionsByDate[dateKey]) {
              summary.sessionsByDate[dateKey] = {
                campIds: [],
                sessionCount: 0
              };
            }
            
            if (!summary.sessionsByDate[dateKey].campIds.includes(camp.id)) {
              summary.sessionsByDate[dateKey].campIds.push(camp.id);
            }
            
            summary.sessionsByDate[dateKey].sessionCount++;
          }
        });
      });
      ...
      summary.todaySessions.sort((a, b) => {
