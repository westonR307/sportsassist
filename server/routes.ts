...
export async function registerRoutes(app: Express) {
  ...
  // Get registrations by camp slug (must be before the :id route to avoid conflicts)
  app.get("/api/camps/slug/:slug/registrations", async (req, res) => {
    try {
      console.log(`Fetching registrations for camp with slug: ${req.params.slug}`);
      
      // First get the camp by slug
      const camp = await storage.getCampBySlug(req.params.slug);
      ...
      if (req.user) {
        ...
        if (isOrgStaff) {
          ...
          const registrations = await storage.getRegistrationsWithChildInfo(camp.id);
