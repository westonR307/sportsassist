...
import type { Camp, Child } from "@shared/schema"
...
import type { ExtendedChild } from "@shared/child-types"
...
const useCampData = (idOrSlug: string | undefined) => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const url = isNumericId ? `/api/camps/${idOrSlug}` : `/api/camps/slug/${idOrSlug}`
...
function CampViewPage(props: { id?: string }) {
  ...
  const heroBgStyle =
    organization && (organization.banner_image_url || organization.bannerImageUrl)
      ? {
  ...
  const isUserRegistered = () => {
    ...
    return registrations.some((reg: any) => reg.parentId === user.id || (reg.parent && reg.parent.id === user.id))
  }
  ...
  const isUserWaitlisted = () => {
    ...
    return registrations.some(
      (reg: any) =>
        (reg.parentId === user.id || (reg.parent && reg.parent.id === user.id)) && reg.status === "waitlisted",
    )
  }
      ...
      <div className="space-y-4">
            ...
            return availableChildren.map((child) => (
              ...
              >
                <CardContent className="p-4 flex items-center justify-between">
                  ...
                  {selectedChildId === child.id && <CheckCircle className="h-5 w-5 text-primary" />}
                </CardContent>
              </Card>
            ))
          })()}
        ...
        {isWaitlist && (
          <Alert>
            ...
            <AlertDescription>
              This camp is currently at capacity. By joining the waitlist, you'll be notified if a spot becomes
              available.
            </AlertDescription>
          </Alert>
        )}
        ...
        {camp?.customRegistrationEnabled && selectedChildId && !isWaitlist && (
          <div className="mt-4">
            <RegistrationCustomFields campId={camp.id} onFieldsChange={setCustomFieldResponses} />
          </div>
        )}
            ...
            <div className="space-y-3">
              {availabilitySlots
                ...
                .map((slot: any) => {
                  ...
                  const dateISOString = !isNaN(date.getTime())
              ...
              {availabilitySlots.filter(
                (slot: any) => !slot.booked && slot.currentBookings < (slot.capacity || slot.maxBookings),
              ).length === 0 && (
                <div className="text-center py-4">
        ...
        <div className="flex justify-end gap-2 mt-4">
          <Button variant="outline" onClick={() => setShowChildSelectionDialog(false)}>
            Cancel
          </Button>
          <Button
            onClick={() => {
    ...
    onSuccess: (data) => {
      ...
      if (data.multipleRegistrations && data.results) {
        ...
        toast({
          ...
          description:
            "You have been added to the waitlist for this camp. We'll notify you if a spot becomes available.",
        })
      } else {
        toast({
    ...
    onError: (error: any) => {
      ...
      if (error.message && error.message.includes("registration has started")) {
        toast({
          ...
          description:
            "This camp cannot be deleted because registration has already started. Please use the cancel option instead.",
          variant: "destructive",
        })
      } else {
        toast({
  ...
  const getLayout = () => {
    // Only return one layout component based on role
    if (isParent) {
      return ParentLayout
  ...
  const renderContent = () => {
        ...
        <div className="flex items-center justify-center h-full">
          <Card className="w-[400px]">
            ...
            <CardContent className="pt-0">
              <p className="text-center text-muted-foreground mb-4">We couldn't find the camp you're looking for.</p>
              <div className="flex justify-center">
                <Button onClick={() => queryClient.invalidateQueries({ queryKey: [`/api/camps/${id}`] })}>Retry</Button>
              </div>
            </CardContent>
          </Card>
        </div>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="relative w-full rounded-lg overflow-hidden shadow-lg" style={heroBgStyle}>
          ...
          <div className="relative z-0 p-6 pb-12 md:p-10 md:pb-16">
            <div className="pt-8 md:pt-10 text-white">
              <h1
                className="text-3xl md:text-4xl lg:text-5xl font-extrabold leading-tight break-words max-w-full drop-shadow-md"
                style={{
            ...
            {hasPermission && (
              <div className="flex flex-wrap gap-2 items-center justify-center">
                ...
                {registrationStatus === "not_open" ? (
                  <Button variant="destructive" onClick={() => setShowDeleteDialog(true)} size="sm">
            ...
            {isParent && !hasPermission && (
              <div className="flex gap-2">
                {isUserRegistered() ? (
                  ...
                  <Button onClick={() => setShowChildSelectionDialog(true)} disabled={registerMutation.isPending}>
                    {registerMutation.isPending ? (
                      <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                    ) : (
                      <CheckCircle className="h-4 w-4 mr-2" />
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6" id="camp-tabs">
          <TabsList className="mb-4 w-full grid grid-cols-2 sm:grid-cols-3 md:flex md:flex-row gap-1 p-1">
            <TabsTrigger value="details" className="text-xs sm:text-sm whitespace-nowrap">
              Details
            </TabsTrigger>
            <TabsTrigger value="registrations" className="text-xs sm:text-sm whitespace-nowrap">
              Registrations
            </TabsTrigger>
            {hasPermission && (
              <TabsTrigger value="attendance" className="text-xs sm:text-sm whitespace-nowrap">
                Attendance
              </TabsTrigger>
            )}
            {showMessagesTab && (
          ...
          <TabsContent value="details" className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="space-y-6">
                <Card
                  className="border-t-[6px] shadow-md"
                  style={{
                    borderTopColor: organization?.primary_color || organization?.primaryColor || "hsl(var(--primary))",
                  }}
                >
                  <CardHeader
                    className="pb-3"
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <CardTitle className="text-xl flex items-center gap-2">
                        <Info
                          className="h-5 w-5"
                          style={{ color: organization?.primary_color || organization?.primaryColor }}
                        />
                        Camp Details
                      </CardTitle>
                      {registrationStatus === "open" && (
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      ...
                      <div className="border rounded-lg p-4">
                        ...
                        <p className="text-muted-foreground">
                          {new Date(camp.registrationStartDate).toLocaleDateString("en-US", {
                            month: "short",
                            day: "numeric",
                          })}{" "}
                          -{" "}
                          {new Date(camp.registrationEndDate).toLocaleDateString("en-US", {
                            month: "short",
                            day: "numeric",
                            year: "numeric",
                          })}
                          ...
                          <div className="space-y-3">
                            ...
                            {availabilitySlots
                              ...
                              .map((slot: any) => {
                                ...
                                return (
                                  <div key={slot.id} className="border rounded-md p-3">
                                    <div className="flex justify-between items-center">
                                      <div>
                                        <p className="font-medium">{formattedDate}</p>
                                        <p className="text-sm text-muted-foreground">
                            ...
                            {availabilitySlots.filter(
                              ...
                              <div className="mt-2 text-center">
                                ...
                                {showAllAvailabilitySlots && !hasPermission ? (
                                  <Button variant="link" onClick={() => setShowAllAvailabilitySlots(false)}>
                                    Show fewer slots
                                  </Button>
                                ) : (
                                  availabilitySlots.filter(
                                    (slot: any) =>
                                      !slot.booked && slot.currentBookings < (slot.capacity || slot.maxBookings),
                                  ).length > 5 && (
                                    <Button
                                      variant="link"
                                      onClick={() => {
                  ...
                  >
                    ...
                    >
                      <div>
                        ...
                        <CardDescription>Session dates and times for this camp</CardDescription>
                      </div>
                      {hasPermission && (
                        <div className="flex">
                          <Button
                      ...
                      {registrations.map((registration: any) => {
                        ...
                        if (camp?.schedulingType === "availability" && slotBookings?.length > 0) {
                          ...
                          bookedSlots = slotBookings.filter((slot: any) => {
                            console.log(
                              `Checking slot ID ${slot.id} for childId ${registration.childId}`,
                              `Bookings:`,
                              JSON.stringify(slot.bookings, null, 2),
                            )
                            ...
                            return (
                              ...
                              slot.bookings.some((booking: any) => {
                                ...
                                const match =
                                  bookingChildId === registration.childId || childObjectId === registration.childId
                          ...
                          console.log(
                            `Found ${bookedSlots.length} booked slots for registration ${registration.id} (childId: ${registration.childId})`,
                          )
                        }

                        return (
                          <div key={registration.id} className="p-3 border rounded-md flex flex-col">
                            <div className="flex justify-between items-center">
                              ...
                              <div className="flex items-center">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span
                                    className={`px-2 py-1 rounded-full text-xs ${registration.paid ? "bg-green-100 text-green-800" : "bg-amber-100 text-amber-800"}`}
                                  >
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {bookedSlots.map((slot: any) => {
                                    ...
                                    const startTime = new Date(
                                      `${slotDate.toISOString().split("T")[0]}T${slot.startTime}`,
                                    )
                                    ...
                                    return (
                                      <span
                                        key={slot.id}
                                        className="px-2 py-1 rounded-md text-xs bg-blue-50 text-blue-700 border border-blue-100"
                                      >
                ...
                <CardContent>
                  {isLoadingRegistrations ? (
                    ...
                    <div className="space-y-6">
                          ...
                          <TableBody>
                            {registrations.map((registration: any) => (
                              <TableRow key={registration.id}>
                                ...
                                <TableCell className="text-center text-sm">{new Date().toLocaleDateString()}</TableCell>
                      ...
                      <div className="flex justify-between items-center">
                        <p className="text-sm text-muted-foreground">Showing {registrations.length} athletes</p>
            ...
            <Dialog open={manageAvailabilityOpen} onOpenChange={setManageAvailabilityOpen}>
              <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>Add, edit, or remove availability slots for this camp.</DialogDescription>
                </DialogHeader>
        ...
        {camp && (
          <>
            <EditCampDialog open={editDialogOpen} onOpenChange={setEditDialogOpen} camp={camp} />
            <Dialog open={scheduleEditorOpen} onOpenChange={setScheduleEditorOpen}>
              <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle>Edit Camp Schedule</DialogTitle>
                  <DialogDescription>Manage your camp schedule with our enhanced calendar view</DialogDescription>
                </DialogHeader>
                {camp && (
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    ...
                    <br />
                    <br />
                    <strong>Note:</strong> Deleting a camp is only possible before registration has started. Once
                    registration has begun, you should cancel the camp instead.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    ...
                    <br />
                    <br />
                    Please provide a reason for cancellation. This information will be shared with registered
                    participants.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <div className="py-4">
          ...
          <Dialog open={showFormFieldsDialog} onOpenChange={setShowFormFieldsDialog}>
            <DialogContent className="max-w-3xl">
              <DialogHeader>
                ...
                <DialogDescription>Customize the registration form fields for this camp.</DialogDescription>
              </DialogHeader>
        ...
        {camp && (
          <CampFormFieldsDialog camp={camp} open={showFormFieldsDialog} onOpenChange={setShowFormFieldsDialog} />
        )}
        ...
        {camp && (
          <DuplicateCampDialog campId={camp.id} open={showDuplicateDialog} onOpenChange={setShowDuplicateDialog} />
        )}
      </div>
    )
  }
